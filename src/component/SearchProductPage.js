...
export default class SearchProductPage extends React.Component {
  ...
  callAPI =_.debounce((text)=>{
    ...
    fetchDataFromAPI(url, "GET", "", null).then(response => {
      ...
      if (response.success == 1) {
        ...
        this.setState({
          searchItems: [],
          message: isStringEmpty(this.searchText) ? strings("SEARCH_START_EMPTY_TEXT") : response.message
        });
      }
    });
  }, 500,);
  ...
  _onEndReached = () => {
    if (this.total > this.state.searchItems.length && !this.IS_API_CALL_ENABLE && this.VIEW_IN_COMPONENT) {
