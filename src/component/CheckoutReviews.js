...
export default class CheckoutReviews extends React.Component {
    ...
    _onPressProceed = () => {
        ...
        firebase.analytics().logEvent(AnalylticsConstant.ECOMMERCE_PURCHACE, analyticsObject);
        fetchDataFromAPI(url, "POST", body, null).then(response => {
            ...
            if (response.success) {
                ...
                if (response.is_stripe) {
                    ...
                    this.props.navigation.navigate("OrderPlaceMessagePage", { OrderId: response.order_id });
                }
            } else {
    ...
    _launchStripeSDK = async(order_id) =>{
        ...
        fetchDataFromAPI(url,"POST",JSON.stringify(body),null).then((response)=> {
          let paymentAPI = response.success ? "stripe/payment/success" : "stripe/payment/cancel"
          paymentAPI = AppConstant.BASE_URL +paymentAPI
          let paymentBody =  { 'order_id' : order_id}
          fetchDataFromAPI(paymentAPI,"POST",JSON.stringify(paymentBody),null).then((paymentResponse)=>{
            this.setState({
              isProgress: false,
            });
            if (paymentResponse.success) {
              this.props.navigation.navigate(
                "OrderPlaceMessagePage",{
                  OrderId : order_id,
                  order_message :  response.success ? StringConstant.OORDER_PLACE_SUBTITLE : "",
                  order_title : response.userCancel ? StringConstant.USER_CANCEL_PAYMENT_MSG : "" ,
                })
            }
          })
        })
            ...
            >
                ...
                {this.state.isLoading ? (
                    <Loading />
                        ...
                        <View style={{ alignSelf: "stretch", flex: 1, backgroundColor: ThemeConstant.BACKGROUND_COLOR_1 }}>
                            ...
                            <KeyboardAvoidingView style={[styles.buttonHeadingView, globleViewStyle]}>
                                ...
                                >
                                    ...
                                    <Text style={styles.orderTotalTextStyle2}>{this.state.cartData.total}</Text>
                                </View>
                                <TouchableOpacity
                                    activeOpacity={1}
